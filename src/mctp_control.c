...
#ifdef MCTP_POSIX_UNIT_TEST
/* Already defined in mctp_control.h */
#else
 ...
 */
void send_completion_response(struct mctp *mctp, uint8_t remote_eid, const void *msg, size_t len, uint8_t completion_code) {
 ...
 */
static int process_set_endpoint_id_control_message(struct mctp *mctp, uint8_t remote_eid, const void *msg, uint16_t len) {
 ...
 * @return int CONTROL_COMPLETE_SUCCESS on success, error code otherwise.
 */
static int process_get_endpoint_id_control_message(struct mctp *mctp, uint8_t remote_eid, const void *msg, uint16_t len) {
 ...
 * @return int CONTROL_COMPLETE_SUCCESS on success, error code otherwise.
 */
static int process_get_mctp_version_support_control_message(struct mctp *mctp, uint8_t remote_eid, const void *msg, uint16_t len) {
 ...
 * @return int CONTROL_COMPLETE_SUCCESS on success, error code otherwise.
 */
int process_get_message_type_support_control_message(struct mctp *mctp, uint8_t remote_eid, const void *msg, uint16_t len) {
 ...
 * @param len Length of the message in bytes.
 */
int send_control_message(struct mctp *mctp, uint8_t eid, const void *msg, size_t msg_len)
{
    ...
    if (msg_len < sizeof(struct control_msg_request)) {
        ...
        send_completion_response(mctp, eid, msg, msg_len, CONTROL_COMPLETE_INVALID_LENGTH);
        return CONTROL_COMPLETE_INVALID_LENGTH;
    }
    ...
    switch (hdr->command_code) {
        case CONTROL_MSG_SET_ENDPOINT_ID:
            completion_code = process_set_endpoint_id_control_message(mctp, eid, msg, msg_len);
            break;
        case CONTROL_MSG_GET_ENDPOINT_ID:
            completion_code = process_get_endpoint_id_control_message(mctp, eid, msg, msg_len);
            break;
        case CONTROL_MSG_GET_MCTP_VERSION_SUPPORT:
            completion_code = process_get_mctp_version_support_control_message(mctp, eid, msg, msg_len);
            break;
        case CONTROL_MSG_GET_MESSAGE_TYPE_SUPPORT:
            completion_code = process_get_message_type_support_control_message(mctp, eid, msg, msg_len);
            break;
        default:
    ...
    if (completion_code != CONTROL_COMPLETE_SUCCESS) {
        ...
        send_completion_response(mctp, eid, msg, msg_len, completion_code);
    }
